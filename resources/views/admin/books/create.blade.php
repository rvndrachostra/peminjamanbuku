...
@section('title', 'Tambah Buku Baru - Book Hub')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Tambah Buku Baru</h1>
    <p class="text-gray-600 mt-2">Daftarkan buku baru ke perpustakaan</p>
</div>
    ...
    <form method="POST" action="{{ route('admin.books.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="year_published" id="year_published" value="{{ old('year_published', date('Y')) }}" required min="1000" max="{{ date('Y') }}"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                ...
                placeholder="Masukkan sinopsis atau deskripsi buku">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
